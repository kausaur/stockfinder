  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-2">
        <Link to={`/stock/${id}`} className="text-gray-500 hover:text-primary">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {peers.map(p => (
                <tr key={p.stockId} className={`hover:bg-gray-50 ${p.stockId === id ? 'bg-blue-50' : ''}`}>
                  <td className="px-6 py-4 whitespace-nowrap sticky left-0 z-10 bg-inherit">
                    <Link to={`/stock/${p.stockId}`} className={`font-semibold ${p.stockId === id ? 'text-primary' : 'text-gray-900'} hover:underline`}>
                      {p.symbol}
                    </Link>
                  </td>
